...
trait Framework {
  ...
  case class EnableAttrIf(val isEnabled: Boolean)
  ...
  implicit def RxBooleanAttrValue(implicit ctx: Ctx.Owner) = new AttrValue[Rx.Dynamic[EnableAttrIf]] {
    def apply(t: Element, a: Attr, r: Rx.Dynamic[EnableAttrIf]): Unit = {
      r.trigger {
        if (r.now.isEnabled) {
          t.setAttribute(a.name, a.name)
        } else {
